  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createJob({
        ...
        status: 'published',
        questions: method === 'internal_ats' ? questions : []
      });
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700">От (Мин)</label>
                <input 
                  type="number" 
                  required
                  className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:border-black focus:ring-black sm:text-sm"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">До (Макс, опц.)</label>
                <input 
                  type="number" 
                  className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 focus:border-black focus:ring-black sm:text-sm"
                  placeholder="3500"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Бенефиты (Компания предлагает)</label>
              <div className="grid grid-cols-2 gap-2">
                {['Жилье', 'Визовая поддержка', 'Обучение', 'Транспорт', 'Питание'].map((b) => (
